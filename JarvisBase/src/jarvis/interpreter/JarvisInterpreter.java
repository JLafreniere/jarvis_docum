...
public class JarvisInterpreter {
	...
	public void message() {
		...
		try {
			...
			throw new IllegalArgumentException(cmd.makeKey() + " is not an object. Type:"+res.getClass()+" , Value: "+res.makeKey());
		}
		...
		// Un message d'avertissement demeure si vous obtenez "ComprendPas". L'interpréteur crash également.
		...
		if (argQueue.size() > 0) {
			...
			if(resMsg instanceof StringAtom){
				...
				if(resStr.getValue().startsWith("ComprendPas")){
					throw new IllegalArgumentException("Object \""+ objectName.makeKey() + "\" of class " + obj.findClassName(this) +" does not understand message \""+selector.makeKey()+"\"");
				}
			}
		}
		/*
